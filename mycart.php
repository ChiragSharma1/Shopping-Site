...
{ $product_id=$rown1['product_id'];
	...
	{	
		...
		if($quantitym1>=$p_qty)
		{
			echo"<input type = 'submit' name = 'btn' value = 'Buy' style = 'width:40px;padding:1px; float:right;margin-right:50px;'/>
		</form>
      </div><br><br><br><br><hr>";
		}
		else{
			echo" <b style='color:red;float:right;width:90px;padding:1px;margin-right:30px;font-size:15px' >OUT of stock</b>
			</form>
      </div><br><br><br><br><hr>";
		}
